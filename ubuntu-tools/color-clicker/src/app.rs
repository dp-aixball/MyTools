...
impl ColorClickerApp {
    ...
    fn detect_and_click(&mut self, ctx: &egui::Context) {
        ...
        if let Some(window_pos) = ctx.input(|i| i.viewport().outer_rect) {
            ...
            let capture_x = (window_pos.center().x * scale - width as f32 * scale / 2.0) as i32;
            let capture_y = (window_pos.max.y * scale) as i32;
            let capture_width = (width as f32 * scale) as u32;
            let capture_height = (height as f32 * scale) as u32;
